...
class PPOAlgo(BaseAlgo):
    ...
    def __init__(self, envs, acmodel, num_frames_per_proc=None, discount=0.99, lr=7e-4, beta1=0.9, beta2=0.999,
                 ...
                 preprocess_obss_teacher=None, preprocess_obss_student=None,
                 reshape_reward=None, class_weights=False, aux_info=None):
        num_frames_per_proc = num_frames_per_proc or 128

        super().__init__(envs, acmodel, num_frames_per_proc, discount, lr, gae_lambda, entropy_coef,
                         value_loss_coef, max_grad_norm, recurrence, preprocess_obss_teacher, preprocess_obss_student,
                         reshape_reward, aux_info)
    ...
    def update_parameters(self, teacher_obs, comm_freq):
        # Collect experiences

        exps, logs = self.collect_experiences(teacher_obs, comm_freq)
        '''
                ...
                for i in range(self.recurrence):
                    ...
                    if self.class_weights:
                        ...
                        class_weights = compute_class_weight(class_weight='balanced',
                                                             ...
                                                             y=actions)
                        class_weights = np.clip(class_weights, 0.0, 3.0)
                        for m in missing:
